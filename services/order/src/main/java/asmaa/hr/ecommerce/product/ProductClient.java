...
import asmaa.hr.ecommerce.order.PurchaseRequest;
...
public class ProductClient {
    ...
    public List<PurchaseResponse> purchaseProducts(List<PurchaseRequest> purchaseRequest) {
        ...
        ResponseEntity<List<PurchaseResponse>> responseEntity = this.restTemplate.exchange(
          this.productUrl + "/purchase",
          POST,
          requestEntity,
          responseType
        );
        if (responseEntity.getStatusCode().isError()){
            throw new BusinessException("An error occured while processing the products purchase");
        }
